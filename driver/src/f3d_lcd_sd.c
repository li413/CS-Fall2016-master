...
Name: Chenyang Li
Username: li413
Partner UserName : zhaok
Partner Name: ke zhao
*/
/* f3d_lcd_sd.c --- 
 * 
...
void f3d_lcd_sd_interface_init(void) {
  /* vvvvvvvvvvv pin initialization for the LCD goes here vvvvvvvvvv
    For the next section, you may find the STM32F3 Data Sheet p43-50, the GPIO Docu
entation, and the intializations in the previous labs helpful.

4.2.1) GPIO Mode
Be sure to select the correct GPIO mode for each of the pins as specified by th
 table above.
 
 
 4.2.2) Configure Alternate Functions

Be sure to select the correct alternate functions for the different pins.
 */ 
  // the ser for out
  ...
  //GPIO_PinOUTConfig(GPIOB,9,GPIO_AF_15);
  // GPIO_PinOUTConfig(GPIOB,10,GPIO_AF_15);
  // GPIO_PinOUTConfig(GPIOB,11,GPIO_AF_15);
  // GPIO_PinOUTConfig(GPIOB,12,GPIO_AF_15);
  // the set for af
  GPIO_StructInit(&GPIO_InitStructure);
...
void f3d_lcd_barFill(uint8_t y_pos, int axis, uint16_t color) { // creates our bar
  uint8_t y;
  uint16_t x[ST7735_width]; 
  for (y = 0; y < ST7735_width; y++){
    if(y < axis / 2) //bar length (divide by two for readability)
      x[y] = color; // changes bar to appropriate color
    else
      x[y] = BLACK; // changes bar back to background color
  }

  //set bar position
  f3d_lcd_setAddrWindow (0,y_pos,ST7735_width-1,ST7735_height-1,MADCTLGRAPHICS);
  for (y=0;y< 10; y++) // 10 pixels south
    f3d_lcd_pushColor(x,ST7735_width); // pushes color to pixels
}
